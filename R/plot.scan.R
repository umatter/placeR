...
##' @usage plot.scan(x, ggmap.zoom=7, ncircles=1, col.grid="white", col.loc="#e55562", col.area="#6b3a70", col.search="#4db0d9", col.circles="#4db0d9", poly.map = NULL )
...
##' @param poly.map spatialPolygonDataframe (optional), map data as basis for the plot. If empty (NULL), the function plots a google map as basis.
  ...
  function(x,
    ...
    if (class(x)!="scan") {
      ...
      if (is.null(poly.map) ){
        ...
        # get necessary data
        data <- x@locations@illustration # data related to the search input/the area searched
        center <- coordinates(gCentroid(poly.map))
        grid.vline <- data$grid.vline
        grid.hline <- data$grid.hline
        area.edges <- data$area.edges
        search.points <- data$search.points
        ...
        # prepare map data for plot with ggplot
        mapdata <- fortify(poly.map)

        # plot the scan object with ggplot

        p <-
          ggplot(mapdata, aes(x=long, y=lat)) +
          geom_map(map=mapdata, aes(map_id=id), fill="white", colour="black") +
          theme_map() +
          coord_equal() +
          geom_vline(aes(xintercept = x), data=grid.vline, colour=col.grid, alpha=.5) +
          geom_hline(aes(yintercept = y), data=grid.hline, colour=col.grid, alpha=.5)+
