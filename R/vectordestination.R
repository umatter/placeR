...
vectordestination <-
  function(longlatpoint, travelvector) {
    Rearth <- 6372795
    Dd <- travelvector$magnitude / Rearth
    Cc <- travelvector$direction

    if (class(longlatpoint) == "SpatialPoints") {
      lata <- coordinates(longlatpoint)[1,2] * (pi/180)
      lona <- coordinates(longlatpoint)[1,1] * (pi/180)
    }
    else {
      lata <- longlatpoint[2] * (pi/180)
      lona <- longlatpoint[1] * (pi/180)
    }
